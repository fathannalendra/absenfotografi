...
if (empty($_GET['filter_bulan'])) {
    ...
    $result = mysqli_query($conn, "SELECT presensi.*, anggota.nama, anggota.kelas FROM presensi JOIN anggota ON presensi.id_anggota = anggota.id WHERE DATE_FORMAT(tanggal_masuk, '%Y-%m') = '$bulan_sekarang' ORDER BY tanggal_masuk DESC");
} else {
    $filter_tahun_bulan = $_GET['filter_tahun'] . '-' . $_GET['filter_bulan'];
    $result = mysqli_query($conn, "SELECT presensi.*, anggota.nama, anggota.kelas FROM presensi JOIN anggota ON presensi.id_anggota = anggota.id WHERE DATE_FORMAT(tanggal_masuk, '%Y-%m') = '$filter_tahun_bulan' ORDER BY tanggal_masuk DESC");
}
...
<div class="modal" id="exampleModal" tabindex="-1">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="<?php echo base_url('admin/presensi/rekap_bulanan_excel.php') ?>">
